...
/**
 * Add a band-limited sawtooth at a given RMS level.
 *
 * A pure sine below the band is a soft target: band-limiting removes it and
 * there is nothing left to argue with. A sawtooth is the honest test, because
 * its harmonics fall off only as 1/n and therefore reach *into* the whistle
 * band however low the fundamental sits — which is exactly what a voice, a
 * violin or a car engine does. Band-limiting still helps enormously (it
 * discards the fundamental, where most of the energy is), but it no longer
 * makes the interferer disappear, so the peak search has to actually win.
 *
 * Band-limited (harmonics summed only up to Nyquist) rather than generated by
 * a naive ramp, because an aliased sawtooth folds energy back down into the
 * whistle band at frequencies that are not harmonics of anything — the test
 * would then be measuring the generator's bugs.
 */
export function addSaw(signal: SynthSignal, hz: number, levelDb: number): SynthSignal {
  const nyquist = signal.sampleRate / 2;
  const harmonics = Math.max(1, Math.floor(nyquist / hz) - 1);

  const raw = new Float32Array(signal.samples.length);
  for (let n = 1; n <= harmonics; n++) {
    const amp = 1 / n;
    const step = (2 * Math.PI * hz * n) / signal.sampleRate;
    let phase = 0;
    for (let i = 0; i < raw.length; i++) {
      raw[i] += amp * Math.sin(phase);
      phase += step;
    }
  }

  const gain = Math.pow(10, levelDb / 20) / (rms(raw) || 1);
  const out = new Float32Array(raw.length);
  for (let i = 0; i < out.length; i++) out[i] = signal.samples[i] + raw[i] * gain;
  return { samples: out, sampleRate: signal.sampleRate, expected: signal.expected };
}
